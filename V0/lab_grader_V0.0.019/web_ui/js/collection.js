...
function wireSelectableFileList({ listElId, files, selectedSet, lastClickedRef, onClickSelect, rerender }) {
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    const overRow = e.target.closest(`#${listElId} [data-select-path]`);
    if (!overRow) return;
  ...
  document.addEventListener('mouseup', () => {
    if (dragActive) { lastClickedRef.idx = dragStartIdx; rerender(); }
    dragStartIdx = null;
...
async function renderCollection() {
  ...
  content.innerHTML = `
    ...
    <p class="page-sub">Pipeline(Collector→Parser→Rule Engine→Scorer→AI→Report) 실행 — 수집한 장비 상태를 target_state/stages 기준과 비교해 채점하고 이력에 저장합니다. 채점 이력이 있어야 '보고서' 탭에서 보고서를 생성할 수 있습니다.</p>
  ...
  document.getElementById('btn-open-log-folder').addEventListener('click', async () => {
    const folder = { original: 'original', analysis: 'problem', masking: 'masking' }[collectionLogSubTab] || 'root';
    const result = await call('open_inspection_log_folder', folder);
