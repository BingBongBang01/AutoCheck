...
try:
    ...
    from engine.comparator import (
        compare_vlan_stage, compare_stp_stage, build_vlan_index,
        compare_lacp_stage, compare_mlag_stage, compare_ospf_stage,
        compare_vrrp_stage, compare_acl_stage,
    )
except ImportError:
    import sys, os
...
def run_extended_stage_rules(project_id, session_id, target_state, collected):
    """
    LACP/MLAG/OSPF/VRRP/ACL 5개 stage 판정. VLAN/STP(run_vlan_stp_rules)와 분리해서
    추가한 이유: 이 5개는 target_state.yaml에 checks가 비어있으면(운영자가 아직 값을 안 채운 lab)
    자동으로 빈 리스트를 반환해야 하므로, 별도 함수로 두는 게 호출부에서 다루기 쉬움.

    collected: {"lacp": {...}, "mlag": {...}, "ospf": {...}, "vrrp": {...}, "acl": {...}}
               각 값은 해당 comparator가 기대하는 파서 결과 형태.
    반환: {"stage_lacp": [Finding,...], "stage_mlag": [...], "stage_ospf": [...],
           "stage_vrrp": [...], "stage_acl": [...]}
    """
    stage_map = [
        ("stage_lacp", "LACP", compare_lacp_stage, collected.get("lacp", {})),
        ("stage_mlag", "MLAG", compare_mlag_stage, collected.get("mlag", {})),
        ("stage_ospf", "OSPF", compare_ospf_stage, collected.get("ospf", {})),
        ("stage_vrrp", "VRRP", compare_vrrp_stage, collected.get("vrrp", {})),
        ("stage_acl", "ACL", compare_acl_stage, collected.get("acl", {})),
    ]
    result = {}
    for stage_id, category, compare_fn, collected_data in stage_map:
        checks = target_state.get(stage_id, {}).get("checks", [])
        verdicts = compare_fn(checks, collected_data) if checks else []
        result[stage_id] = [Finding.from_verdict(project_id, session_id, category, v) for v in verdicts]
    return result
