...
print(f"\n[4] 인코딩 확인 (Windows에서 한글 깨짐/실행 오류의 주 원인)")
import locale
print(f"    시스템 기본 인코딩: {locale.getpreferredencoding()}")
print(f"    콘솔 출력 인코딩: {sys.stdout.encoding}")
try:
    test_path = os.path.join("labs", "lab1_campus", "target_state.yaml")
    if os.path.exists(test_path):
        with open(test_path, encoding="utf-8") as f:
            f.read()
        print("    설정 파일(UTF-8) 읽기: OK")
except UnicodeDecodeError as e:
    print(f"    [!] UTF-8 읽기 실패: {e}")
    print("    코드에 encoding='utf-8'이 누락된 곳이 있을 수 있음 — 최신 버전인지 확인하세요")

print(f"\n[5] 실행 커맨드 확인")
print("    Windows PowerShell/cmd:  python main.py")
...
if not missing and not missing_pkgs:
    print("환경 이상 없음 — python main.py 실행해보세요(실장비 접속 필요)")
else:
