...
class LogViewerApiMixin:
    ...
    def open_inspection_log_folder(self):
        """'점검 로그 뷰어'의 'Open Folder' 버튼 — 현재 활성 L2 프로파일 폴더
        (data/<고객사>/<프로파일>/)를 OS 네이티브 파일 탐색기로 연다."""
        try:
            self._project()
        except RuntimeError:
        ...
        log_storage.open_in_file_explorer(log_paths["root"])
        return {"ok": True, "path": log_paths["root"]}
