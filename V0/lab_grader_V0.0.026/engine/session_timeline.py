...
def write_session_timeline(session_dir, out_name="_event_timeline.json", window_minutes=DEFAULT_WINDOW_MINUTES):
    ...
    쓰기는 StorageService로 위임(원자적 쓰기 + 로깅) — 이 함수는 여전히 session_dir
    문자열/경로 하나만 받는 얇은 어댑터라 collector.py 등 기존 호출부는 그대로 쓸 수 있다."""
    ...
    storage_service.save_json(PathTarget(path=Path(session_dir)), out_name, timeline)
    return timeline
