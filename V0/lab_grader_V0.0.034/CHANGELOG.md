...
## v0.0.034 (현재) — 시작 최적화 · 삭제된 로그 재등장 수정 · 드래그 자동 스크롤 · 중지 시 자동 저장 · 클라우드 AI 제공자 확장

폴더명 = `VERSION` = CHANGELOG 제목을 같은 값으로 유지하는 규칙(위 '번호 체계 통일' 참고)에 따라
이 폴더에서 한 작업은 모두 v0.0.034 항목에 모아 둔다.
