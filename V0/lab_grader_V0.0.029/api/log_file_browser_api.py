    ...
    def delete_log_files(self, paths):
        ...
        for path in paths or []:
            ...
            try:
                ...
                # 원본과 사본(세션 터미널 점검) 동시 삭제 처리
                fname = os.path.basename(abs_path)
                twin_path = None
                abs_orig = os.path.abspath(original_dir) if original_dir else None
                abs_sess = os.path.abspath(session_dir) if session_dir else None
                
                if abs_orig and abs_path.startswith(abs_orig):
                    if abs_sess: twin_path = os.path.join(abs_sess, fname)
                elif abs_sess and abs_path.startswith(abs_sess):
                    if abs_orig: twin_path = os.path.join(abs_orig, fname)
                
                if twin_path and os.path.isfile(twin_path):
                    try:
                        os.remove(twin_path)
                    except OSError:
                        pass
