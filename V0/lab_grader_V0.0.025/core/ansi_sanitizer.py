...
def clean_terminal_log(raw_text):
    ...
    # 네트워크 장비는 거의 항상 CRLF(\r\n)로 줄을 끝냄 — 이 \r은 "덮어쓰기"가 아니라
    # 그냥 줄바꿈의 일부라서 먼저 \n으로 정규화해둬야 한다. 정규화 없이 아래에서 \n으로
    # split하면 각 조각이 "...내용\r" 형태로 남고, _apply_carriage_returns가 그걸
    # "\r로 나뉜 마지막 조각"(빈 문자열)만 남겨버려서 실제 내용이 전부 빈 줄로 지워진다.
    # (show run/show spanning-tree 등 여러 줄 출력이 통째로 빈칸으로 저장되던 버그의 원인)
    text = text.replace("\r\n", "\n")
    lines = []
