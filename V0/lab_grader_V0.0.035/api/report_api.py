...
def _latest_terminal_log_paths_by_device(project_id):
    """labs/{project}/terminal_sessions/ 의 장비별 최신 파일 1개씩 {device: (mtime, path)} 로 반환.
    ...
    if not os.path.isdir(session_dir):
        return {}
    latest = {}
    for path in glob.glob(os.path.join(session_dir, "*_*_*_*.txt")):
        ...
        if body.startswith("AutoCheck_"):
            ...
            device = parts[0] if len(parts) == 3 else body_no_prefix
        else:
            parts = body.split("_", 3)
            device = parts[3] if len(parts) == 4 else body
...
class ReportApiMixin:
    ...
    def generate_excel_report(self, template_path=None):
        ...
        out_path = str(AppPaths.labs_root() / project_id / f"{ts}_report_{project_id}.xlsx")
    ...
    def generate_pptx_report(self, device_name=None, template_path=None):
        ...
        out_path = str(AppPaths.labs_root() / project_id / f"{ts}_report_{target_device}.pptx")
