...
function renderLogViewer() {
  ...
  el.innerHTML = `
    <div style="display:flex;gap:12px;height:360px;">
      <div style="width:260px;flex:0 0 260px;overflow:auto;background:var(--card);border:1px solid var(--border);border-radius:var(--radius-sm);padding:6px;">
        ${collectionLogFiles.map(f => `
          <div class="connection-device log-file-row ${collectionOpenTabs.includes(f.path) ? 'selected' : ''}" data-path="${f.path}" style="cursor:pointer;">
  ...
  el.querySelectorAll('.log-file-row').forEach(row => {
