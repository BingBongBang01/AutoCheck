...
_inspection_job = {"running": False, "done": False, "log": []}
...
def _run_one_session_inspection(session_id, commands, results_dir):
    ...
    fname = os.path.join(results_dir, f"{device}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
...
class TerminalApiMixin:
    ...
    def run_terminal_inspection(self, session_ids):
        ...
        with _inspection_lock:
            _inspection_job.update({"running": True, "done": False, "log": [f"점검 시작 — 대상 {len(valid_ids)}개 세션, 커맨드 {len(commands)}개"]})
