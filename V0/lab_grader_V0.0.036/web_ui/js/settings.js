...
async function renderSettings() {
  ...
  content.innerHTML = `
    ...
    <p class="page-sub">
      이 페이지의 설정은 <b>모든 고객사·모든 정기점검 회차에 공통</b>으로 적용됩니다.
      고객사나 회차를 바꿔도 그대로 유지됩니다.
      회차마다 다르게 두어야 하는 값(장비 목록, 수집 커맨드, 점검 항목)은 각 탭에서 관리합니다.
    </p>
    <div class="card">
    ...
    <div class="card section-gap">
      ...
      <div style="margin-top:12px;display:flex;gap:8px;align-items:center;">
        <!-- 필드 순서는 위 저장된 행과 동일하게 [제공자] [이름] [모델] [API 키] — 한쪽만 바꾸지 말 것
             (settings-cloud-api.js의 renderCloudApiList 주석 참고). 모델 목록은 제공자를
             바꿀 때 wireAddCloudApiButton()이 다시 채운다. -->
        ...
        <select class="field" data-role="model" id="new-cloud-api-model" style="max-width:230px;" title="사용할 모델"></select>
        <input class="field" data-role="model-custom" id="new-cloud-api-model-custom" placeholder="모델 ID 직접 입력" style="max-width:230px;display:none;">
        <input class="field" id="new-cloud-api-endpoint" placeholder="API 주소 (예: https://.../v1/chat/completions)" style="min-width:260px;flex:1;display:none;">
