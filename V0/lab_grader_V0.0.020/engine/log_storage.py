...
CMD_CATALOG_DIR = "03_CMD"
DEVICE_LIST_DIR = "04_device_list"
GENERAL_LOG_DIR = "99_log"
...
def get_customer_dir(customer_name):
    return os.path.join(get_app_root(), "data", safe_folder_name(customer_name))


def get_profile_dir(customer_name, profile_name):
    return os.path.join(get_customer_dir(customer_name), safe_folder_name(profile_name))


def get_cmd_catalog_dir(customer_name, profile_name):
    """명령어 카탈로그 export/import 기본 위치: data/<고객사>/<프로파일>/03_CMD."""
    d = os.path.join(get_profile_dir(customer_name, profile_name), CMD_CATALOG_DIR)
    os.makedirs(d, exist_ok=True)
    return d


def get_device_list_dir(customer_name, profile_name):
    """장비목록 export/import 기본 위치: data/<고객사>/<프로파일>/04_device_list."""
    d = os.path.join(get_profile_dir(customer_name, profile_name), DEVICE_LIST_DIR)
    os.makedirs(d, exist_ok=True)
    return d


def get_general_log_dir(customer_name, profile_name):
    """00~04번 전용 폴더에 속하지 않는 그 외 모든 로그(수집/채점 원본, running-config
    스냅샷 등)의 기본 저장 위치: data/<고객사>/<프로파일>/99_log."""
    d = os.path.join(get_profile_dir(customer_name, profile_name), GENERAL_LOG_DIR)
    os.makedirs(d, exist_ok=True)
    return d
