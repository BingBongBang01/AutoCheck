...
def _try_api(scored_or_findings, api_cfg, user_approved_cloud=False, hostnames=None):
    ...
    from core.sanitizer import mask_findings
    ...
    masked = mask_findings(findings, hostnames=hostnames) if _looks_like_findings(scored_or_findings) else findings
    prompt_source = build_context(masked, max_items=20) if _looks_like_findings(scored_or_findings) else masked
