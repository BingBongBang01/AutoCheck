...
class Api:
    ...
    def get_dashboard(self):
        ...
        inv = di.load_inventory(
            paths["device_inventory"],
            paths["lab_meta"],
            paths["ip_allocation"]
        )
        ...
        if not latest:
            return {
                "kpi": {
                    ...
                    "reachable": 0,
                    "offline": 0,
                    "running": len(enabled_devices),
                    "sessions": 0,
                },
                "stages": [],
                "ai_summary": "아직 채점 이력이 없습니다.",
            }
        ...
        critical = sum(
            1 for a in ai_result["all_anomalies"]
            if a["result"] == "FAIL"
        )
        warning = sum(
            1 for a in ai_result["all_anomalies"]
        ...
        sessions_count = len(
            glob.glob(f"history/{project_id}/*.json")
        )
