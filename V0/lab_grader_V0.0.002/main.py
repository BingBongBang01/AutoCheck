...
def load_lab_config():
    with open(f"{LAB_DIR}/target_state.yaml") as f:
        target_state = yaml.safe_load(f)
    with open(f"{LAB_DIR}/stages.yaml") as f:
...
def grade(collect_fn):
    ...
    print(f"\n[저장됨] {path}")
    if prev:
        print(f"[참고] 직전 회차({prev['session']}) 대비 비교는 diff 기능(로드맵)에서 지원 예정")
...
def real_collect():
    ...
    results, errors, session_dir = collect_all(
        f"{LAB_DIR}/lab_meta.yaml", f"{LAB_DIR}/ip_allocation.yaml", commands
    )
    if results is None:
        return None
    if errors:
