...
## v0.5.6 (현재) — 프로파일별 상태 영속화, 실시간 오류 분석 재설계, 로그 인프라 정비

- **고객사/프로파일 전환 및 복원 (`engine/project_manager.py`, `api/project_core_api.py`)**
  - `active_project.yaml`에 고객사별 `last_profile_by_customer`를 함께 저장 — 고객사/정기점검 팝업에서
    고객사를 클릭하면 그 고객사에서 마지막으로 쓰던 회차가 바로 열린다(회차 목록을 다시 뒤질 필요 없음)
  - `ensure_active_profile()` 추가 — 프로그램 시작 시 저장된 활성 프로파일 id가 실제로 존재하는지 먼저
    확인하고, 폴더가 사라졌거나 비어 있으면 고객사별 마지막 사용 기록으로 되살린다(`core-init.js`가
    상태바를 그리기 전에 호출 — '프로파일 없음'으로 뜬 채 방치되던 문제 해결)
- **실시간 감시 — 프로파일별 상태 영속화 및 CRT 세션 재접속 대응 (`engine/realtime_monitor.py`, `core/crt_stream_watcher.py`, `api/log_analysis_run_api.py`)**
  - `RealtimeMonitor.snapshot()`/`restore()` 추가 — 경고 이력·체크리스트 판정·로그 버퍼를 프로파일 폴더의
    `realtime_state.json`에 저장해, 프로그램을 껐다 켜도 그 프로파일에서 찾은 오류가 그대로 남는다
  - CRT가 이미 열려 있는 상태에서 프로그램을 실행해도 최신 로그의 끝부분을 되짚어 판정(토스트 없이,
    `이전 기록`으로 표시)하고, 세션 재접속으로 로그 파일이 바뀌면 이전 파일을 닫고 새 파일로 자연스럽게
    이어서 추적(장비 단위로 판정 상태를 들고 있어 파일이 바뀌어도 끊기지 않음)
  - 장비 식별에 실패한 로그 파일은 추적 대상에서 제외(더 이상 tail하지 않음) — 상단 경고 줄 대신
    `파일 진단` 버튼 자체를 강조색으로 표시
- **실시간 오류 분석 UI 재설계 (`web_ui/js/realtime-monitor-panel.js`, `style.css`)**
  - 좌 1/4(오류 목록) · 우 3/4(선택한 오류 상세) 2분할 레이아웃으로 전면 재구성
  - 같은 기술 분류(VLAN/인터페이스/정적 라우팅/BGP·OSPF 인접/링크/STP·MLAG/운영 명령)로 여러 장비의
    오류를 묶어 `<분류> / <N>대` 형태로 표시 — 어느 분류에도 안 걸리던 정보성 경고까지 전부 분류에
    포함시켜 "카운트는 있는데 목록은 비어 보이는" 누락을 없앰
  - 우측 하단 경고 토스트를 없애고 상단바 알림 버튼 + 우측 슬라이드 패널로 교체 — 경고가 화면을
    가리고 사라지는 대신 목록에 계속 쌓여 나중에 되짚을 수 있음
- **로그 인프라 — 전량 보존 및 회차별 그룹 표시 (`core/app_logger.py`, `web_ui/js/log-run-groups.js`)**
  - 메모리 로그 버퍼가 레벨과 무관하게 전부(최대 2만 줄) 보관하도록 변경 — 화면 표시 레벨을 나중에
    낮춰도 이미 쌓인 로그가 소급 적용되어 보이고, 파일에는 실행부터 종료까지 모든 레벨이 계속 기록됨
  - 점검 로그/원본로그분석/마스킹 세 탭의 로그 목록을 실행(run) 단위로 묶는 공용 컴포넌트 추가 —
    "N분 전 점검 로그"처럼 접이식 그룹으로 표시하고, 그룹을 펼쳐야 그 회차의 장비별 로그가 나열됨
