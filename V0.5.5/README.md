...
- 🔄 **회차별 장비 목록 자동 승계 & 고객사별 마지막 프로파일 기억 (v0.5.6)**: 동일 고객사의 이전 정기점검 회차 장비/IP/계정 정보를 자동으로 물려받고, 고객사/정기점검 팝업에서 고객사를 고르면 그 고객사에서 마지막으로 열었던 회차가 바로 열림
- 📡 **SecureCRT 세션 로그 실시간 감시**: `CRTlog/`에 기록되는 CLI 입·출력을 0.3초 간격 tail로 따라가며 사전 점검 결과(Baseline) 대비 설정 삭제·링크/인접 DOWN을 즉시 감지하고, 장비별 체크리스트와 원인 분석을 실시간 갱신. 발견된 오류는 기술 분류(VLAN/인터페이스/링크/BGP·OSPF 인접/STP·MLAG 등)로 묶어 `<분류> / <N>대` 형태로 보여주며, 프로파일별 감시 상태(경고 이력·체크리스트)를 파일로 저장해 프로그램을 껐다 켜도 유지됨(v0.5.6)
- 🔔 **알림 패널 & 회차별 로그 그룹 표시 (v0.5.6)**: 실시간 경고는 상단바 알림 버튼 → 우측 슬라이드 패널에서 이력으로 확인하며, 점검 로그/분석/마스킹 목록은 실행(run) 단위로 접이식 그룹화되어 "N분 전 점검 로그"처럼 표시
...
- ⚡ **고성능 비동기 로깅 & UI 스트리밍**: Queue 기반 디스크 비동기 로거가 실행부터 종료까지 모든 레벨을 파일에 전량 기록하고, 메모리에도 레벨 구분 없이 보관(v0.5.6)해 화면 표시 레벨을 나중에 낮춰도 이미 쌓인 로그가 소급 적용됨 — 200ms DOM Throttling, Delta streaming(`since_index`), 영속적 레벨 필터링(`INFO`/`WARN`/`ERROR`/`DEBUG`)
...
* **9. 점검 로그**: 수집된 원본 CLI 로그 확인, 이상 탐지 및 외부 제출용 데이터 마스킹을 수행합니다.
  로그 목록은 실행(run) 단위로 접이식 그룹화되어 "N분 전 점검 로그"처럼 표시되며(v0.5.6), 그룹을
  펼쳐야 그 회차의 장비별 로그가 나열됩니다 — 회차가 쌓여도 원하는 시점을 바로 찾을 수 있습니다.
...
* **11. 전체 로그**: 실시간 실행 로그를 200ms 속도제어(Throttling) 및 레벨 필터(`INFO`/`WARN`/`ERROR`/`DEBUG`)로 스트리밍 확인하고 전체 세션 로그(.txt)를 내보냅니다. 백그라운드에서는 항상 모든 레벨을 기록하므로(v0.5.6),
  화면 필터를 나중에 낮춰도 이미 쌓인 로그가 그 수준으로 다시 보이고 이후 로그도 이어서 표시됩니다.
...
[SecureCRT 터미널 작업]
      │ 세션 로그 append
[Documents/AutoCheck/CRTlog/*.txt · *.log]
      │ 바이트 오프셋 감시(0.3초) — 새로 덧붙여진 부분만
[core/crt_stream_watcher.py]
      │ 파일 → 장비 판정 (engine/stream_device_matcher.py)
[engine/baseline_diff_engine.py] ◄── [engine/baseline_store.py] 00_orignal_log 기준 스냅샷
      │
[engine/realtime_monitor.py] ──► 화면 3분할 (로그 / 체크리스트 / 원인 분석) + 상단바 알림 패널
      │                          + realtime_state.json(프로파일별 스냅샷)
```

### 화면 구성
- **좌측**: 장비별 실시간 CLI 캡처. `탭 보기`는 한 장비씩, `분할 보기`는 장비 박스를 위아래로 쌓습니다.
  각 장비 박스는 스크롤되지 않고 최신 줄만 보여주며, 박스 목록 전체만 스크롤됩니다.
- **우측 상단 — 실시간 오류 분석 (v0.5.6 재설계)**: 좌 1/4(오류 목록) · 우 3/4(선택한 오류 상세) 2분할.
  단순 나열이 아니라 **같은 기술 분류(VLAN/인터페이스/정적 라우팅/BGP·OSPF 인접/링크/STP·MLAG/운영 명령)로
  여러 장비를 묶어** `<분류> / <N>대` 형태로 보여줍니다(예: MLAG peer-link 이상이 4대에서 났으면
  `STP/MLAG / 4대` 한 줄). 목록에서 고르면 오른쪽에 장비별 원인 추정·권고 조치·근거 원문·해당 장비
  체크리스트가 전부 펼쳐지고, 길면 그 열만 스크롤됩니다.
- **우측 하단**: 장비 × 7개 항목(VLAN / 인터페이스 / 정적 라우팅 / 라우팅 인접 / 링크 / STP·MLAG / 위험 명령)
  체크리스트. 이상 항목이 위로 정렬되고, 그룹을 클릭하면 세부 경고 이력이 열립니다.
- 좌우·상하 **구분선은 드래그로 비율 조절**되며, 그 값과 보기 모드·선택 장비는
  `config/realtime_watch.yaml`에 저장돼 프로그램을 다시 켜도 유지됩니다.
- **프로그램 실행 시 자동 시작** 체크박스를 켜면 앱을 띄우는 즉시 감시가 시작됩니다.
- **알림 패널 (v0.5.6)**: 실시간 경고는 우측 하단 토스트 대신 상단바 알림 버튼에 쌓입니다. 버튼을
  누르면 오른쪽에서 슬라이드 패널이 열려 심각도·장비·원인·근거를 카드로 확인할 수 있고, 복구된 경고는
  지워지지 않고 `해제` 표시로 남습니다.
- **프로파일별 상태 영속화 (v0.5.6)**: 경고 이력·체크리스트 판정·로그 버퍼가 프로파일 폴더의
  `runs/<run_id>/realtime_state.json`에 주기 저장돼, 프로그램을 껐다 켜도 그 프로파일에서 찾은 오류가
  그대로 복원됩니다. SecureCRT가 이미 열려 있던 상태로 프로그램을 켜면 최신 로그의 끝부분을 되짚어
  판정하고(토스트 없이 `이전 기록`으로 표시), 세션을 껐다 다시 열어 로그 파일이 바뀌면 이전 파일을 닫고
  새 파일로 자연스럽게 이어서 추적합니다.
- **우클릭 컨텍스트 메뉴**: 로그 박스, 체크리스트, 분석 결과 항목을 우클릭하여 특정 장비나 규칙의 경고를 숨김 처리할 수 있습니다.
- **고정 카드 및 표시 설정**: 중요한 항목(예: 특정 장비의 특정 장애)을 상단에 고정 카드로 띄울 수 있으며, 설정 모달을 통해 관리할 수 있습니다.

### 감지 항목
| 구분 | 감지 대상 | 판정 |
|---|---|---|
| 설정 삭제 | `no vlan` · `no ip route` · `no interface` · `no neighbor` · `no router bgp` | Baseline에 있던 대상이면 CRITICAL |
| 인터페이스 차단 | `shutdown` (config 문맥에서 대상 인터페이스 추적) | Baseline 등록 인터페이스면 CRITICAL |
| 위험 운영 명령 | `reload` · `write erase` · `erase startup-config` 등 | CRITICAL |
| 상태 변화 | `BGP/OSPF-5-ADJCHANGE` · `LINEPROTO-5-UPDOWN` · `MLAG` · `STP` | DOWN이면 CRITICAL, 복구면 WARNING |

### 로그 파일 ↔ 장비 매칭
SecureCRT는 세션 로그 파일명을 장비명이 아니라 **접속 IP**로 남기는 경우가 많고(`192.168.205.101_20260803_152204.txt`),
기본 파일명이 `session.log`인 환경도 있습니다. 그래서 다음 순서로 판정합니다:

1. 파일명 토큰이 장비 목록의 장비명과 일치
2. 파일명에 든 IP가 장비 목록의 관리 IP와 일치
3. 로그 내용의 `! device: X` 헤더 (`show running-config` 출력에 항상 있음)
4. 로그 내용의 프롬프트 `X#` / `X>`

그래도 실패하면 **파일 진단** 버튼으로 파일별 매칭 근거를 확인할 수 있습니다(v0.5.6부터 상단 안내 줄
대신 이 버튼 자체가 강조색으로 바뀝니다). 식별에 실패한 파일은 이번 점검 대상이 아닌 것으로 보고
**추적하지 않습니다** — 매 tick 재시도는 계속하므로, 접속 직후 프롬프트가 늦게 찍혀 실패한 파일도
프롬프트가 찍히는 순간 자동으로 감시 대상에 들어옵니다.

> **참고**: 감시를 시작하기 전부터 파일에 있던 내용은 화면에만 흐리게 채워지고(`--:--:--` 표시) 경고
> 판정도 하되(v0.5.6) **토스트/알림은 띄우지 않습니다**(`이전 기록` 배지로만 구분). 프로그램을 켜기
> 전부터 SecureCRT가 열려 있던 경우가 흔해서, 그 세션에서 이미 벌어진 변경(설정 삭제·링크 DOWN)을
> 놓치지 않기 위함입니다. 감시 중에 새로 열린 세션 로그는 첫 줄부터 실시간 판정하며, SecureCRT를
> 껐다 다시 열어 로그 파일이 바뀌어도(재접속) 이전 파일을 닫고 새 파일로 자연스럽게 이어서 추적합니다.
...
- **후속 과제**: 구조적 리팩토링 항목 및 기술 부채 관련사항은 [ARCHITECTURE.md](ARCHITECTURE.md)의 *Known follow-ups* 섹션을 참고하세요.
...
- **개발자 작업 가이드**: [DEVELOPER_GUIDE.md](DEVELOPER_GUIDE.md)
- **워크스페이스 데이터 배치**: [WORKSPACE_STRUCTURE.md](WORKSPACE_STRUCTURE.md)
