"""
전체 파이프라인 진입점.
본인 노트북에서: python3 main.py grade  (실제 collector.collect_all 사용)
장비 접속 없이 테스트: python3 main.py grade --mock  (재현 데이터로 파이프라인만 검증)
...
def grade_via_pipeline(collect_fn):
    ...
    from pipeline.steps import (CollectorStep, ParserStep, RuleEngineStep, ScorerStep,
                                  ScoreboardPrintStep, HistoryStep, AIAnalysisStep, ReportStep)
    from core.context import ProjectContext, SessionContext
    import datetime

    target_state, stages_cfg = load_lab_config()
    project_ctx = ProjectContext(project_id=LAB_NAME, paths={"target_state": f"{LAB_DIR}/target_state.yaml"})
...
def real_collect():
    ...
    load_inventory(paths["device_inventory"], paths["lab_meta"], paths["ip_allocation"])
...
def mock_collect():
    """장비 접속 없이 파이프라인만 검증하는 재현 데이터 (D12 세션 기록 기준)."""
    _, stages_cfg = load_lab_config()
    commands = get_all_commands(stages_cfg)
    vlan_cmd = next(c for c in commands if c.startswith("show vlan"))
    stp_cmd = next(c for c in commands if c.startswith("show spanning-tree vlan"))
...
200   SERVER                           active
"""
    def stp_combo(root_vlan1=False, root_vlan100=False, root_vlan200=False, prio=32768):
        return f"""
VLAN0001
  Bridge ID  Priority    {prio}
{'             This bridge is the root' if root_vlan1 else ''}
VLAN0100
  Bridge ID  Priority    {prio}
{'             This bridge is the root' if root_vlan100 else ''}
VLAN0200
  Bridge ID  Priority    {prio}
{'             This bridge is the root' if root_vlan200 else ''}
VLAN0999
  Bridge ID  Priority    {prio}
"""

    return {
        "Core1":   {vlan_cmd: vlan_brief, stp_cmd: stp_combo()},
        "Core2":   {vlan_cmd: vlan_brief, stp_cmd: stp_combo(root_vlan1=True)},
        "Agg1":    {vlan_cmd: vlan_brief, stp_cmd: stp_combo()},
        "Agg2":    {vlan_cmd: vlan_brief, stp_cmd: stp_combo()},
        "Access1": {vlan_cmd: vlan_brief, stp_cmd: stp_combo(root_vlan100=True)},
        "Access2": {vlan_cmd: vlan_brief, stp_cmd: stp_combo()},
        "Access3": {vlan_cmd: vlan_brief, stp_cmd: stp_combo(root_vlan200=True)},
    }
...
if __name__ == "__main__":
    ...
    if "--mock" in sys.argv:
        if "--pipeline" in sys.argv:
            grade_via_pipeline(mock_collect)
        else:
            grade(mock_collect)
    else:
        if "--pipeline" in sys.argv:
