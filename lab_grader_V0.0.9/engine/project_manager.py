...
def list_projects():
    ...
    for entry in sorted(os.listdir(LABS_DIR)):
        ...
        if os.path.exists(meta_path):
            ...
            meta = {"display_name": entry, "created_at": datetime.date.today().isoformat()}
...
def create_project(display_name):
    ...
    with open(os.path.join(path, "project_meta.yaml"), "w", encoding="utf-8") as f:
        yaml.dump({"display_name": display_name, "created_at": datetime.date.today().isoformat()},
...
def project_paths(project_id):
    ...
    return {
        ...
        "commands_catalog": os.path.join(base, "commands_catalog.yaml"),
