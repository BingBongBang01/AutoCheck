...
## v0.5.9 (현재) — 정기점검 보고서 오탐 정리: '점검 불가/대상 아님'을 '비정상'과 분리
...
- **표준 네트워크 다이어그램 기호·규칙을 따른다** (`engine/topology_svg.py`)
  ...
  - 같은 노드에서 같은 방향으로 나가는 링크를 기호 가장자리에 **부채꼴로 펼친다** — 한 점에서
    ...
    거의 항상 그렇게 된다). 라벨은 선을 따라 빈자리를 찾아 놓는다(아래 '랩과 다른 구성' 참고).
...
- **회귀 테스트** `tests/test_topology.py`(45건) — 픽스처는 실제 워크스페이스 출력을 그대로 쓴다.
  상상해서 만든 벤더 출력으로 테스트하면 현장에서 한 줄도 못 읽는 파서가 통과한다(이 프로젝트의
  기존 BGP/OSPF 파서가 실제로 그 상태였다).

#### 랩과 다른 구성에서 그림이 깨지던 것 수정

구성도를 만들 때 본 구성은 Core/Agg/Access 3계층 랩 하나뿐이었다. 현장에 있는 다른 구성
(링, 스타, 스파인-리프, 체인, 같은 계층 풀메시, 계층을 건너뛰는 직결, 30대 규모, 시스코 명명,
끌어 옮긴 배치) 21가지를 로그 형태로 만들어 돌려 보니 **구조 판정은 전부 맞았지만 그림이 읽히지
않는 경우가 있었다.** 예외·비결정성·XML 오류·노드 겹침은 없었다.

- **같은 계층 링크와 계층을 건너뛰는 링크가 중간 장비를 관통했다** — 링 구성의 첫↔마지막 링크가
  사이의 장비 4대를 가로지르고, 풀메시 5대에서는 10건이 그랬다. 계층형 배치는 위아래 이웃
  계층만 곧게 이을 수 있다. 이제 **직선이 남의 기호를 지나면 비껴가는 곡선으로 그린다**
  (`engine/topology_svg.py` 의 `_route`). 2차 베지어는 `2t(1-t)·(C-M)` 만큼 휘므로 '이 장비를
  이만큼 비켜야 한다'를 제어점 거리로 바로 환산할 수 있고, 덜 휘어도 되는 쪽을 고르므로
  결정적이다. 휜 곡선이 또 다른 장비에 걸리면 훑어 보고 더 크게 돌린다.
- **팬아웃이 큰 장비에서 포트 라벨이 한 자리에 쌓였다** — Core 1대에 Access 12대면 라벨 4~8개가
  같은 자리에 겹치고 일부는 기호 위에 얹혔다(30대 구성에서 35쌍). 고정 위치 대신 **선을 따라
  후보 자리를 여러 개 두고 이미 놓인 라벨·기호와 겹치지 않는 첫 자리를 고른다**(`_LabelPlacer`).
  순서가 고정돼 있어 그림은 여전히 결정적이다. 중앙 라벨(Po 이름·설명)도 같은 방식으로 바뀌어
  기존의 '중점이 겹치면 어긋나게' 규칙을 대체한다.
- **Po 이름 없이 접힌 병렬 링크에 개수가 없었다** — 시스코의 `show interfaces status` 에는 Po
  소속 열이 없어 병렬 링크가 이름 없는 묶음으로 접힌다. 그러면 굵은 선 하나로 보여 몇 가닥인지
  도면에서 사라졌다. 이제 이름이 없어도 `×2` 를 적는다.
- **왼쪽·위로 끌어 놓은 장비가 잘려 사라졌다** — 좌표가 음수가 되는데 SVG 는 (0,0)부터 그리고,
  그 좌표가 저장되므로 다시 열어도 안 보였다. 캔버스 안으로 붙여 세운다
  (`engine/topology_layout.py`). 아래로 끌었을 때 **범례가 그 장비를 덮던 것**도 함께 고쳤다 —
  캔버스를 늘릴 때 범례 자리를 확보하고, 그래도 겹치면 범례가 다른 모서리로 옮겨 간다.
- **장비명 위로 선이 지나가 이름을 못 읽었다** — 자기 장비에서 아래로 나가는 링크는 반드시 이름
  줄을 지난다. 장비명·IP 에 배경색 테두리를 둘렀다(`paint-order`, `web_ui/style.css`). 선을
  이름까지 피하게 해 보았더니 촘촘한 Access 줄에서 크게 돌다가 오히려 다른 기호를 지나갔다.
- **이중화 쌍 상자가 사이의 다른 장비까지 감쌌다** — 쌍이 나란히 있지 않으면(계층 판정이
  엇갈렸거나 사용자가 끌어 옮겼다) '이 셋이 한 쌍'으로 읽힌다. 그럴 땐 상자를 그리지 않는다.
...
## v0.5.8 — 실시간 감시 '파일 진단' 모달에 다중 선택/삭제/폴더 열기 추가
