...
class LogAnalysisRunApiMixin:
    ...
    def _realtime_watch_devices(self, device_names=None):
        """감시 대상 장비명 — 인자로 받은 목록(연결 탭에서 체크된 장비)이 없으면
        장비 목록(Device Inventory)에서 활성화된 전체를 쓴다."""
        if device_names:
            return [str(n) for n in device_names if n]
        try:
            targets = self.get_terminal_targets() or []
        except Exception:
            targets = []
    ...
    # ---------- 스트림 수신 ----------
    def _on_crt_stream_delta(self, device, text, path):
        """CRTStreamWatcher 콜백(워커 스레드) — 차분 텍스트를 화면 버퍼에 넣고 판정해 push."""
        engine = getattr(self, "_baseline_diff_engine", None)
        if engine is None:
            return
        # 장비 목록에서 선택되지 않은 장비의 로그는 무시한다(CRTlog 폴더에는 과거 세션 로그도 섞여 있다).
