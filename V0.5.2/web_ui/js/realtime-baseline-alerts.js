...
// 버튼 라벨/색을 현재 감시 상태에 맞춘다(탭을 다시 그릴 때마다 호출).
async function syncRealtimeWatchButton(btn) {
  ...
  btn.innerHTML = `<span class="material-symbols-rounded">${running ? 'sensors' : 'sensors_off'}</span>` +
    (running ? `실시간 감시 중${status.alert_count ? ` (${status.alert_count})` : ''}` : '실시간 감시 시작');
  btn.title = running
    ? `CRTlog를 0.3초 간격으로 tail하며 Baseline과 대조 중 — 기준 장비 ${(status.devices || []).length}대`
    : 'SecureCRT 세션 로그를 실시간으로 감시해 Baseline 대비 변경을 알립니다';
}
