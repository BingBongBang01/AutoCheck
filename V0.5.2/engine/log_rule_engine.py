...
class Correlator:
    ...
    def _apply(self, rule, findings):
        ...
        # 인터페이스(또는 rule이 지정한 key)별로 모아 조건을 본다.
        buckets = {}
        for f in findings:
            if f["rule_id"] not in required and not any(
                    tag in required for tag in [f.get("category_tag", "")]):
                continue
            key = extract_interface(f["block"][0] if f["block"] else "") or "(전역)"
