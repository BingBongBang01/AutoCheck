...
class LocalAiApiMixin:
    @property
    def AI_CONFIG_PATH(self):
        """앱 전역 설정 — 경로는 AppPaths로 고정한다(core/app_settings.py 주석 참고)."""
        from core.app_settings import ai_config_path
    ...
    MODEL_PARAM_DEFAULTS = {
        ...
        # 클라우드 모델 — 여기 없는 모델은 CloudAiApiMixin.CLOUD_PROVIDER_TYPES의 제공자별
        # batch_chars/max_tokens로 폴백한다(_batch_defaults 참고). 그래서 제공자/모델을
        # 추가할 때 이 표에 줄을 늘릴 필요가 없고, 특정 모델만 다르게 두고 싶을 때만 적는다.
        "claude-haiku-4-5": {"batch_chars": 15000, "max_tokens": 1000},
        "gemini-2.5-flash-lite": {"batch_chars": 35000, "max_tokens": 1200},
        "gemini-2.5-pro": {"batch_chars": 45000, "max_tokens": 1800},
        "gpt-4o-mini": {"batch_chars": 25000, "max_tokens": 1000},
        "solar-mini": {"batch_chars": 15000, "max_tokens": 1000},
    }

    # 로컬 모델명의 파라미터 크기(예: "gemma-3n-4b" -> 4, "llama-3.2-3b" -> 3.2)에 따라
    # VRAM 사용량에 맞는 컨텍스트 오버플로우 방지 값을 동적으로 산출한다.
    # 경계값은 각 구간의 상한 기준(<=)이며, 구간을 벗어나는 값은 가장 가까운 구간에 맞춘다.
    LOCAL_SIZE_TIERS = [
        (3, {"batch_chars": 7500, "batch_segs": 4, "max_tokens": 4096}),   # 2B ~ 3B
        (6, {"batch_chars": 5000, "batch_segs": 3, "max_tokens": 3072}),   # 4B ~ 6B
        (8, {"batch_chars": 3000, "batch_segs": 2, "max_tokens": 2048}),   # 7B ~ 8B
    ]
    LOCAL_SIZE_TIER_DEFAULT = {"batch_chars": 1500, "batch_segs": 1, "max_tokens": 1536}  # 16B~

    def _parse_local_model_size_b(self, model_id):
        """모델명에서 'b' 앞의 파라미터 크기(단위: B)를 추출. 못 찾으면 None."""
        import re
        matches = re.findall(r'(\d+(?:\.\d+)?)\s*b(?![a-z0-9])', (model_id or "").lower())
        if not matches:
            return None
    ...
    def get_model_param_defaults(self, model_id):
        ...
        모델명에서 파라미터 크기(B)를 읽어낼 수 있으면 크기 구간별 기본값을 동적으로 계산하고,
        그렇지 않은 모델(클라우드 모델 등)은 정적 테이블을 그대로 사용한다."""
        size_b = self._parse_local_model_size_b(model_id)
        if size_b is not None:
            for upper_bound, defaults in self.LOCAL_SIZE_TIERS:
                if size_b <= upper_bound:
                    return defaults
            return self.LOCAL_SIZE_TIER_DEFAULT
    ...
    def save_local_ai_config(self, settings):
        ...
        if new_model != prev_model:
            defaults = self.get_model_param_defaults(new_model)
            if defaults:
                cfg["local_batching"] = {
