...
class BaselineStore:
    ...
    # ---------- 로드 ----------
    def load_baseline(self, customer, profile, original_dir=None):
        """활성 프로파일의 00_orignal_log/*.txt 전부를 읽어 장비별 스냅샷을 메모리에 올린다.
        같은 장비의 로그가 여럿이면 mtime이 가장 최신인 파일 하나만 기준으로 삼는다.
        반환: {"ok": True, "devices": [장비명...], "loaded": n}
        """
        if original_dir is None:
            from engine import log_storage
            original_dir = log_storage.get_profile_log_paths(customer, profile)["original"]
