# AutoCheck — 네트워크 장비 자동 점검/채점 시스템 (v0.5.3)
...
- 📡 **SecureCRT 세션 로그 실시간 감시 (v0.5.3)**: `CRTlog/`에 기록되는 CLI 입·출력을 0.3초 간격 tail로 따라가며 사전 점검 결과(Baseline) 대비 설정 삭제·링크/인접 DOWN을 즉시 토스트로 경고하고, 장비별 체크리스트와 원인 분석을 실시간 갱신
