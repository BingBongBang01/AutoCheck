...
본 프로젝트는 단순한 네트워크 정기점검 자동화 프로그램이 아니라,

**Enterprise Network Inspection Platform** 구축을 목표로 개발 중이다.

Parser → Rule → Finding → AI → Report 구조를 기반으로
실무 네트워크 엔지니어의 정기점검 업무를 자동화하는 것을 목표로 한다.
...
> **신규(v0.0.8) — Device Inventory 아키텍처 개편 + Material Design 3 웹 UI.** CHANGELOG.md 참고.
> - `python gui_web.py` — Material Design 3 웹 UI(권장, pywebview 필요: `pip install pywebview`)
> - `python gui.py` — 기존 Tkinter 데스크톱 UI(표준 라이브러리만)
...
## v0.0.9 — Pipeline 아키텍처 (실행 방법 추가)

```bash
python main.py --mock --pipeline   # 신규 Pipeline 경로 (권장, 기존과 결과 동일 검증됨)
python main.py --mock              # 기존 경로 (하위호환용, 당분간 유지)
```

신규 패키지: `core/`(Finding/Context), `pipeline/`(Step/실행기), `plugins/vendors/`
(VendorDriver, Arista 우선), `plugins/parsers/registry.py`(Parser 자동탐색),
`rule_engine/rules.py`(comparator를 Finding으로 승격하는 어댑터).
