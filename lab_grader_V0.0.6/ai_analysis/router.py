...
def _try_api(scored, api_cfg):
    """
    실제 LLM API 호출 시도. api_cfg 예: {"provider": "anthropic", "endpoint": ..., "api_key_env": "ANTHROPIC_API_KEY"}
    네트워크/키 문제 시 예외를 던져서 라우터가 다음 단계로 넘어가게 함.
    """
    import os
    ...
    return {"source": "api", "summary": text, "anomaly_count": len(rule_based.detect_anomalies(scored)),
...
def analyze(scored, ai_config=None):
    """
    ai_config 예:
    ...
    for provider in providers:
        try:
            if provider.get("type") == "api":
                return _try_api(scored, provider)
