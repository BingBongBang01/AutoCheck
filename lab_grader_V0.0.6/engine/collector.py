...
def collect_device(node_name, commands, ip_allocation, session_dir, on_update=None,
                    retry_count=1, retry_delay_sec=5, config_snapshot_dir=None, lab_name=None):
    ...
    for attempt in range(1, retry_count + 1):
        ...
        try:
            ...
            # --- Config Snapshot: running-config가 수집 대상에 포함돼 있으면 별도 폴더에 이력 보관 ---
            if config_snapshot_dir:
                ...
                if running_cfg:
                    ...
                    snap_path = os.path.join(config_snapshot_dir, f"{node_name}_{today}.txt")
    ...
    return node_name, None, last_error
...
def collect_all(lab_meta_path, ip_allocation_path, commands, connection_path="connection.yaml", max_workers=None):
    ...
    retry_count = ssh_cfg.get("retry_count", 1)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                collect_device, name, commands, ip_allocation, session_dir,
                retry_count=retry_count, retry_delay_sec=retry_delay_sec,
                config_snapshot_dir=config_snapshot_dir, lab_name=lab_name,
            ): name
