...
// xterm(xterm.js 약 490KB + addon + css)은 index.html에서 미리 받지 않는다 — 시작할 때마다
// 터미널을 쓰지도 않는데 파싱 비용을 내던 부분이라 이 페이지를 처음 열 때만 주입한다.
// 두 번째 호출부터는 같은 Promise를 그대로 돌려주므로 중복 로드가 없다.
let xtermReady = null;
...
function ensureXterm() {
  ...
  // addon-fit은 window.Terminal이 먼저 있어야 하므로 순차 로드한다(병렬 금지).
  xtermReady = loadScript('vendor/xterm/xterm.js')
