...
class InspectionReportApiMixin:
    ...
    def get_inspection_report_context(self):
        ...
        return {
            ...
            "reports_dir": str(builder.reports_dir(customer, profile)),
            # 최신 회차의 원본 로그 폴더(없으면 빈 문자열) — 'Open Folder' 버튼용.
    ...
    def export_inspection_pdf_report(self, options=None):
        """'PDF 보고서 생성' 버튼 — 7개 실제 정기점검 PDF를 분석해 만든 표준 양식으로
        data/<고객사>/<프로파일>/reports/ 에 저장한다. 데이터 조립은 엑셀과 동일
        (build_context)하고 표지/장비목록/지원이력 등 표준 전용 필드만 옵션으로 더 받는다.
        options: export_inspection_report()의 옵션 전부 + {site_name, vendor, report_title,
                  confirmer:{company,name}}."""
        target = self._report_target()
        if target is None:
            return {"error": "활성 고객사/정기점검 프로파일이 없습니다."}
        customer, profile = target
        options = options or {}
        try:
            result = builder.export_pdf_report(
                customer, profile,
                project_id=self._project(),
                filename=options.get("filename") or None,
                inspection_date=options.get("inspection_date") or None,
                manager=options.get("manager") or None,
                inspector=options.get("inspector") or None,
                confirmer=options.get("confirmer") or None,
                site_name=options.get("site_name") or None,
                vendor=options.get("vendor") or None,
                report_title=options.get("report_title") or None,
                devices_filter=options.get("devices") or None,
            )
        except InspectionReportError as exc:
            return {"error": str(exc)}
        except RuntimeError as exc:
            # register_fonts()가 한글 TTF를 못 찾으면 여기로 온다(맑은 고딕/나눔고딕 미설치 환경).
            return {"error": str(exc)}
        except OSError as exc:
            return {"error": f"보고서 파일을 저장할 수 없습니다({exc}). 같은 이름의 파일이 "
                              f"PDF 뷰어에서 열려 있으면 닫고 다시 시도하세요."}
        return {"ok": True, **result}
    ...
    def delete_inspection_report(self, name):
        """보고서 .xlsx 삭제 — 경로 탈출을 막기 위해 파일명만 받는다.
        list_reports()가 프로파일 reports/와 회차별 runs/<run_id>/reports/를 함께 보여주므로,
        삭제도 같은 폴더 집합에서 찾아야 한다(목록에는 있는데 지워지지 않는 항목 방지)."""
        target = self._report_target()
        if target is None:
            return {"error": "활성 고객사/정기점검 프로파일이 없습니다."}
        ...
        for row in builder.list_reports(*target):
            if os.path.basename(row["path"]) != filename:
                continue
            try:
                os.remove(row["path"])
            except OSError as exc:
