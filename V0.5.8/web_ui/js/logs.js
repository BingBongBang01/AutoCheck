...
async function renderLogs() {
  ...
  content.innerHTML = `
    ...
    <p class="page-sub">프로그램의 모든 동작/오류 로그입니다. 백그라운드에서는 DEBUG를 포함한 <b>모든 로그를 항상</b> 세션 로그 파일에 기록합니다
      (프로그램 실행부터 종료까지). 레벨 필터를 바꾸면 <b>이미 쌓인 로그도</b> 그 수준으로 다시 보이고 이후 로그도 이어서 표시됩니다.
      랙 방지를 위해 화면에만 최근 500줄을 200ms 속도제어로 표출하며, 화면에서 밀려난 줄도 파일에는 그대로 남아 내보내기(.txt)로 받을 수 있습니다.</p>
    <div class="card">
...
function updateLogsMeta(result) {
  ...
  // 화면 줄 수와 세션 전체 줄 수를 함께 보여준다 — 화면에 500줄만 있는 것이 '나머지가 사라진
  // 것'으로 읽히면 안 된다(전부 파일에 남아 있고, 내보내기로 그대로 받을 수 있다).
  const buffered = result && result.buffered_count ? ` | 메모리 보관: ${result.buffered_count}줄` : '';
  meta.textContent = `화면 표출: 최근 ${logsDomLines.length}줄 (필터: ${levelText})${buffered}`
    + ` | 세션 전체: ${logsFullCount || logsTotalCount}줄 (모두 파일에 기록됨)${pauseStatus}`;
}
