...
앱의 최대 CPU 소비처는 **로그 판정 엔진**(`engine/log_rule_engine.py`)이고, 그중 `match_signature()`가 `evaluate()` 시간의 약 75%다. 다만 이 병목의 해법으로 흔히 떠오르는 **정규식 통합(31개 → 단일 alternation)은 실측 결과 2.6배 느려서 폐기**했다. 실제로 성립하는 해법은 **줄 단위 메모화**이며(6.9~9.0배, 결과 동일), 31개 signature 전부가 `scope=None`이라 순수 함수임을 확인해 안전성이 확보됐다.
...
| 테스트 파일 수 | **0개** | `find -name 'test_*.py'` | 이번 세션 |
...
- **대상 파일**: `tests/test_log_rule_engine_golden.py` (신규)
- **지금 동작**: 테스트가 0개다. 판정 결과가 바뀌어도 아무도 모른다.
- **변경**: `engine/log_rule_engine.py` 상단 주석에 이미 실제 오탐/미탐 사례가 문장 단위로 나열돼 있다. 이것을 골든 케이스로 고정한다.
  - 오탐이어야 하는 줄(판정 = None): `"Number of table drops    : 0"`, `"hitless-reload-down   Disabled   300"`, `"Interfaces that will be enabled at the next timeout:"`, `"   U - In Use    D - Down"`
  - 미탐이어야 하는 줄(판정 ≠ None): `"NTP is disabled."`, `"% Invalid input"`, `"% Unavailable command"`, `"The system rebooted due to unknown reasons."`, counters errors 표의 0 아닌 칸
  - `analyze_text()` 전체 출력(findings의 `rule_id`/`severity`/`line_no` 튜플 리스트)을 합성 코퍼스에 대해 스냅샷으로 고정
- **예상 이득**: 성능 이득 없음. 2단계의 전제 조건.
- **리스크**: LOW
- **공수**: S
- **검증 방법**: 현재 코드에서 전부 통과해야 한다. 통과하지 않으면 그 케이스는 현재 동작을 잘못 기술한 것이므로 테스트를 고친다(코드를 고치지 않는다 — 이 단계의 목적은 현 동작 고정이다).
- **선행 조건**: 없음

### 0-2. 재현 가능한 벤치마크 하네스

- **대상 파일**: `tools/bench_log_analysis.py` (신규)
- **지금 동작**: 성능 측정이 매번 임시 스크립트로 이뤄져 비교 불가.
- **변경**: 고정 시드 합성 로그 생성기(장비 수·중복률·syslog 타임스탬프 분산을 파라미터로) + `analyze_text()` / `evaluate()` 단계별 타이밍 + `RealtimeMonitor.state()` payload 크기를 한 번에 출력. 회귀 임계값(예: 기준선 대비 +10% 이상 느려지면 실패)을 포함한다.
- **예상 이득**: 성능 이득 없음. 이후 모든 항목의 이득을 숫자로 주장할 근거.
- **리스크**: LOW
- **공수**: S
- **검증 방법**: 같은 시드로 두 번 돌려 편차가 5% 이내인지 확인.
- **선행 조건**: 없음
...
- **변경**: 두 함수를 줄 문자열 키로 메모화한다. **안전성 근거**: 31개 signature 전부 `scope=None`임을 실측 확인했다(`0/31`). `match_signature()`의 유일한 ctx 의존은 `if scope and not scope.search(ctx.command)`이므로 scope가 없으면 ctx를 전혀 읽지 않는다 — 줄만의 순수 함수다. `find_keyword()`는 애초에 ctx를 받지 않는다.
  ...
  - `self._memo_safe` 가드가 핵심이다. 앞으로 누군가 `log_rules.json`에 `scope`를 가진 signature를 추가하면 메모가 자동으로 꺼지고 정확성이 유지된다.
- **예상 이득**: **`match_signature` 6.9배(25→4 ms), `find_keyword` 9.0배(18→2 ms)** — 중복 68% 코퍼스 실측. 결과 동일함을 전 줄에 대해 확인했다. `evaluate()` 전체로는 대략 2~3배 개선이 기대되나(suppressor 59 ms는 그대로 남는다), 실제 이득은 로그 중복률에 비례하므로 0-2 하네스로 중복률별 곡선을 기록한다.
- **리스크**: MEDIUM — 판정 로직 경로에 캐시를 넣는 변경이다. `_memo_safe` 가드와 0-1 골든 테스트가 함께 있어야 한다. 메모리는 고유 줄 수 × 줄 길이이므로 상한이 필수다.
- **공수**: M
- **검증 방법**: 0-1 골든 테스트 전부 통과 + 메모 on/off로 `analyze_text()` 결과가 완전히 동일함을 합성 코퍼스와 실제 수집 로그 양쪽에서 확인 + 0-2 하네스로 속도 기록.
- **선행 조건**: **0-1, 0-2**
...
1. **0단계**: 골든 테스트가 존재하고 현재 코드에서 전부 통과한다. 벤치마크 하네스가 같은 시드로 편차 5% 이내로 재현된다.
...
4. **2-1**: 중복률 68% 코퍼스에서 `match_signature` **5배 이상**, `find_keyword` **7배 이상** 개선. `analyze_text()` findings가 메모 on/off에서 **완전히 동일**(합성 코퍼스 + 실제 수집 로그 양쪽). 메모 엔트리가 상한을 넘지 않는다.
